...
The functions contained in this module allow the user to analyse Molecular Dynamics (MD) trajectories of straight edge dislocations 
generated using LAMMPS.

The script "ovito_disloc.py" post-processes dislocation trajectories generated as LAMMPS dumps files containing the unwrapped coordinates of atoms.
The functions in this script use as input the text files generated by "ovito_disloc.py", the format of which is described in further detail in the 
its documentation.

The function "track_disloc" included here allows the user to track dislocations across the various frames of a trajectory, since in a simulation containing 
multiple dislocations, different dislocations are indexed differently by the DXA tool. Accordingly, this function allows the user to "track" a given
dislocation throught a trajectory and analyse its dynamics.

The script "dislocation_script.py" shows how the functions included in this module can be used to process dislocation trajectories prior to carrying out
the dislocation tracking.
"""

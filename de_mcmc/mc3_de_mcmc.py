...
directory should be set by setting the out_dir variable, where the DE-MC output can be stored. The script mBF_new.py can be used to read in analyze the results of the fitting procedure, 
giving the parameter marginal and joint posterior distributions. In addition to parameter distributions, other outputs of the analysis script include trace plots and parameter mean and 
...
set at the beginning of this script). de_mcmc.npz file contains the DE_MC accepted samples and is used for further analysis, while the de_mcmc.log and de_mcmc_statistics.txt files contain 
